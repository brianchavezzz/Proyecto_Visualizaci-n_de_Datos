...
def read_data():
    ...
    bd.insert(0, "Num. de intercambios", 1)
    bd['País']=[(a[0:3]) for a in bd['OportunidadAsignada']]
    return bd
df=read_data()
...
promedio_hotel["lat_c"]=promedio_hotel["Campus"].map(lat)
promedio_hotel["lon_c"]=promedio_hotel["Campus"].map(lon)

mapa = px.scatter_mapbox(promedio_hotel, lat="lat_c", lon="lon_c", color='Num. de intercambios',size="Num. de intercambios",
...
#streamlit run grafi_interna.py